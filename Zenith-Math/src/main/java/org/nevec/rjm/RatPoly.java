...
/**
 * A uni-variant polynomial with rational coefficients. Alternatively to be interpreted as a sequence which has the
 * polynomial as an (approximate) generating function.
 * 
 * @since 2006-06-25
...
class RatPoly {
	/**
	 * The list of all coefficients, ascending exponents. Starting with a0, then a1, representing a value
	...
	/**
	 * Default ctor. Initializes the zero-valued polynomial x=0.
	 * 
	 * @author Richard J. Mathar
	 */
	public RatPoly() {
	 ...
	 * 
	 * @param L
	 *            the coefficients a0, a1, a2, a3,.., A deep copy of the these is created.
	 * @author Richard J. Mathar
	 */
	...
	public RatPoly(final Vector<BigInteger> A, final Vector<BigInteger> B, int nmax) {
		/*
		 * To allow common initialization with the signature below, the main body is assembled in a separate function.
		 */
		init(A, B, nmax);
	}
	 ...
	 * 
	 * @param A
	 *            the list of values in the numerator of AFB. At least one of these values must be a negative integer,
	 *            which implicitly determines the order of the new polynomial.
	 * @param B
	 *            the list of values in the denominator of AFB
	 * @since 2009-08-05
	 * @author Richard J. Mathar
	 ...
	 * 
	 * @param n the zero-based index of the coefficient. n=0 for the constant term. If the polynomial has not yet the
	 * degree to need this coefficient, the intermediate coefficients are implicitly set to zero.
	 * 
	 ...
	 * 
	 * @param n
	 *            the zero-based index of the coefficient. n=0 for the constant term. If the polynomial has not yet the
	 *            degree to need this coefficient, the intermediate coefficients are implicitly set to zero.
	 * @param value
	 ...
	 * 
	 * @return The smallest exponent n such that [x^n] of the polynomial is nonzero. If the polynmial is identical zero,
	 ...
	 * @param val
	 *            the factor
	 * @return the product of this with the factor. All coefficients of this have been multiplied individually by the
	 *         factor.
	 * @author Richard J. Mathar
	 */
	...
	public RatPoly pow(final int n) throws ArithmeticException {
		...
		if (n < 0)
			...
			/*
			 * this ought probably be done with some binary representation of the power and a smaller number of
			 * multiplications.
			 */
			for (int i = 1; i <= n; i++)
	...
	/**
	 * Raise to a rational power. The result is the taylor expansion of this, truncated at the first term that remains
	 * undetermined based on the current number of coefficients.
	 * 
	 * @param r
	...
	public RatPoly divide(final RatPoly val, int nmax) {
		...
		RatPoly resul = new RatPoly();
		/*
		 * todo: If the polynomial division is exact, we could leave the loop earlier, indeed
		 */
		for (int n = 0; n <= nmax; n++) {
	 ...
	 * @param val
	 *            the other polynomial
	 * @return A vector with [0] containg the polynomial of degree which is the difference of thisdegree and the degree
	 *         of val. [1] the remainder polynomial. This = returnvalue[0] + returnvalue[1]/val .
	 * @since 2012-03-01
	 * @author Richard J. Mathar
	 */
	public RatPoly[] divideAndRemainder(final RatPoly val) {
		...
		if (thisSimpl.degree() < valSimpl.degree()) {
			...
			/*
			 * long division. Highest degree by dividing the highest degree of this thru val.
			 */
			ret[0] = new RatPoly();
			ret[0].set(thisSimpl.degree()
					- valSimpl.degree(), thisSimpl.a.lastElement().divide(valSimpl.a.lastElement()));

			/*
			 * recurrences: build this - val*(1-termresult) and feed this into another round of division. Have
			 * intermediate ret[0]+ret[1]/val.
			 */
	 ...
	 * 
	 * @return The representation a0,a1,a2,a3,... This is a sort of opposite of the ctor that takes a string as an
	 *         argument.
	 * @since 2008-10-25
	 * @author Richard J. Mathar
	 */
	 ...
	 * 
	 * @return To representation a0+a1*x+a2*x^2+... This does not print the terms with coefficients equal to zero.
	 * @since 2008-10-26
	 * @author Richard J. Mathar
	 */
	...
	/**
	 * Simplify the representation. Trailing values with zero coefficients (at high powers) are deleted. This modifies
	 * the polynomial on the stop (does not return another instance)
	 * 
	 * @author Richard J. Mathar
	 */
	 ...
	 * @return The polynomial with all coefficients beyond deg set to zero. If newdeg =3, for example the polynomial
	 *         returned has at most degree 3. If newdeg is larger than the degree of this, zeros (at the higher orders
	 *         of x) are appended. That polynomial would have formal degree larger than this.
	 * @since 2008-10-26
	 * @author Richard J. Mathar
	 */
	...
	public Vector<BigComplex> roots(int digits) {
		...
		/*
		 * iterate until convr indicates that all values changed by less than the digits precision indicates.
		 */
		boolean convr = false;
		while (!convr) {
			convr = true;
	 ...
	 * 
	 * @return The vector of integer roots, with multiplicity. The shows alternatingly first a root then its
	 *         multiplicty, then another root and multiplicty etc.
	 * @since 2008-10-26
	 * @author Richard J. Mathar
	 */
	public Vector<BigInteger> iroots() {
		...
		if (lowd == 0 && a.elementAt(0).compareTo(BigInteger.ZERO) == 0) {
			/*
			 * Case of polynomial identical to zero: reported as a simple root of value 0.
			 */
		...
		/*
		 * multiply all coefs with the lcm() to get an integer polynomial start with denominator of first non-zero
		 * coefficient.
		 */
		...
		/*
		 * collect the integer roots (multiple roots only once). Since we removed the zero already above, cand does not
		 * contain zeros.
		 */
