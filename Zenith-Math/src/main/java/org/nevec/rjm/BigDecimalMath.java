...
/**
 * BigDecimal special functions. <a href="http://arxiv.org/abs/0908.3030">A Java Math.BigDecimal Implementation of Core
 * Mathematical Functions</a>
 * 
 * @since 2009-05-22
 * @author Richard J. Mathar <a href="http://apfloat.org/">apfloat</a> <a href="http://dfp.sourceforge.net/">dfp</a>
 *         <a href="http://jscience.org/">JScience</a>
 */
public class BigDecimalMath {

	/**
	 * The base of the natural logarithm in a predefined accuracy. http://www.cs.arizona.edu/icon/oddsends/e.htm The
	 * precision of the predefined constant is one less than the string's length, taking into account the decimal dot.
	...
	static public BigDecimal gamma(MathContext mc) {
		...
		if (mc.getPrecision() < GAMMA.precision())
			...
			/*
			 * how many terms: zeta-1 falls as 1/2^(2n+1), so the terms drop faster than 1/2^(4n+2). Set 1/2^(4kmax+2) <
			 * eps. Leading term zeta(3)/(4^1*3) is 0.017. Leading zeta(3) is 1.2. Log(2) is 0.7
			 */
			int kmax = (int) ((Math.log(eps / 0.7) - 2.) / 4.);
			mcloc = new MathContext(1 + err2prec(1.2, eps / kmax));
			for (int n = 1;; n++) {
				/*
				 * zeta is close to 1. Division of zeta-1 through 4^n*(2n+1) means divion through roughly 2^(2n+1)
				 */
	 ...
	 * @param mc
	 *            The required mathematical precision.
	...
	static public BigDecimal sqrt(final BigDecimal x, final MathContext mc) {
		...
		for (;;) {
			/*
			 * s = s -(s/2-x/2s); test correction s-x/s for being smaller than the precision requested. The relative
			 * correction is 1-x/s^2, (actually half of this, which we use for a little bit of additional protection).
			 */
			...
			/*
			 * debugging System.out.println("itr "+x.round(locmc).toString() + " " + s.round(locmc).toString()) ;
			 */
		}
		return s;
	} /* BigDecimalMath.sqrt */
	 ...
	 * @param x
	 *            The argument.
	 * @return The cubic root of the BigDecimal rounded to the precision implied by x. The sign of the result is the
	...
	static public BigDecimal root(final int n, final BigDecimal x) {
		...
		for (;;) {
			/*
			 * s = s -(s/n-x/n/s^(n-1)) = s-(s-x/s^(n-1))/n; test correction s/n-x/s for being smaller than the
			 * precision requested. The relative correction is (1-x/s^n)/n,
			 */
	...
	static public BigDecimal hypot(final BigDecimal x, final BigDecimal y) {
		...
		/*
		 * truncate to the precision set by x and y. Absolute error = 2*x*xerr+2*y*yerr, where the two errors are 1/2 of
		 * the ulp's. Two intermediate protectio digits.
		 */
	...
	static public BigDecimal hypot(final int n, final BigDecimal x) {
		...
		/*
		 * Truncate to the precision set by x. Absolute error = in z (square of the result) is |2*x*xerr|, where the
		 * error is 1/2 of the ulp. Two intermediate protection digits. zerr is a signed value, but used only in
		 * conjunction with err2prec(), so this feature does not harm.
		 */
	 ...
	 * @param x
	 *            the argument.
	 * @return exp(x). The precision of the result is implicitly defined by the precision in the argument. In particular
	 *         this means that "Invalid Operation" errors are thrown if catastrophic cancellation of digits causes the
	 *         result to have no valid digits left.
	 * @since 2009-05-29
	 * @author Richard J. Mathar
	 */
	static public BigDecimal exp(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			...
			/*
			 * Relative error in inverse of invx is the same as the relative errror in invx. This is used to define the
			...
			/*
			 * recover the valid number of digits from x.ulp(), if x hits the zero. The x.precision() is 1 then, and
			 * does not provide this information.
			 */
			return scalePrec(BigDecimal.ONE, -(int) (Math.log10(x.ulp().doubleValue())));
		} else {
			/*
			 * Push the number in the Taylor expansion down to a small value where TAYLOR_NTERM terms will do. If x<1,
			 * the n-th term is of the order x^n/n!, and equal to both the absolute and relative error of the result
			 * since the result is close to 1. The x.ulp() sets the relative and absolute error of the result, as
			 * estimated from the first Taylor term. We want x^TAYLOR_NTERM/TAYLOR_NTERM! < x.ulp, which is guaranteed
			 * if x^TAYLOR_NTERM < TAYLOR_NTERM*(TAYLOR_NTERM-1)*...*x.ulp.
			 */
			...
			if (Math.pow(xDbl, TAYLOR_NTERM) < TAYLOR_NTERM * (TAYLOR_NTERM - 1.0) * (TAYLOR_NTERM - 2.0) * xUlpDbl) {
				...
				/*
				 * TAYLOR_NTERM terms to be added means we move x.ulp() to the right for each power of 10 in
				 * TAYLOR_NTERM, so the addition won't add noise beyond what's already in x.
				 */
				...
				/*
				 * exp(x+deltax) = exp(x)(1+deltax) if deltax is <<1. So the relative error in the result equals the
				 * absolute error in the argument.
				 */
				...
				/*
				 * Final powering by 10 means that the relative error of the result is 10 times the relative error of
				 * the base (First order binomial expansion). This looses one digit.
				 */
				MathContext mc = new MathContext(expxby10.precision() - exSc);
				/*
				 * Rescaling the powers of 10 is done in chunks of a maximum of 8 to avoid an invalid operation response
				 * by the BigDecimal.pow library or integer overflow.
				 */
				while (exSc > 0) {
	...
	static public BigDecimal exp(final MathContext mc) {
		...
		if (mc.getPrecision() < E.precision())
			...
			/*
			 * Instantiate a 1.0 with the requested pseudo-accuracy and delegate the computation to the public method
			 * above.
			 */
	 ...
	 * @param x
	 *            the argument.
	 * @return ln(x). The precision of the result is implicitly defined by the precision in the argument.
	 * @since 2009-05-29
	 * @author Richard J. Mathar
	 */
	static public BigDecimal log(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			/*
			 * The standard Taylor series around x=1, z=0, z=x-1. Abramowitz-Stegun 4.124. The absolute error is
			 * err(z)/(1+z) = err(x)/x.
			 */
			...
			/*
			 * Map log(x) = log root[r](x)^r = r*log( root[r](x)) with the aim to move roor[r](x) near to 1.2 (that is,
			 * below the 0.3 appearing above), where log(1.2) is roughly 0.2.
			 */
			int r = (int) (Math.log(xDbl) / 0.2);

			/*
			 * Since the actual requirement is a function of the value 0.3 appearing above, we avoid the hypothetical
			 * case of endless recurrence by ensuring that r >= 2.
			 */
			r = Math.max(2, r);
			...
			/*
			 * error propagation: log(x+errx) = log(x)+errx/x, so the absolute error in the result equals the relative
			 * error in the input, xUlpDbl/xDbl .
			 */
	...
	static public BigDecimal log(int n, final MathContext mc) {
		...
		if (n <= 0)
			...
			if (mc.getPrecision() < LOG2.precision())
				...
				/*
				 * Broadhurst <a href="http://arxiv.org/abs/math/9803067">arXiv:math/9803067</a> Error propagation: the
				 * error in log(2) is twice the error in S(2,-5,...).
				 */
			...
			/*
			 * summation of a series roughly proportional to (7/500)^k. Estimate count of terms to estimate the
			 * precision (drop the favorable additional 1/k here): 0.013^k <= 10^(-precision), so k*log10(0.013) <=
			 * -precision so k>= precision/1.87.
			 */
			...
			/*
			 * log3 is roughly 1, so absolute and relative error are the same. The result will be divided by 12, so a
			 * conservative error is the one already found in mc
			 */
			...
			/*
			 * summation of a series roughly proportional to (7/160)^k. Estimate count of terms to estimate the
			 * precision (drop the favorable additional 1/k here): 0.046^k <= 10^(-precision), so k*log10(0.046) <=
			 * -precision so k>= precision/1.33.
			 */
			...
			/*
			 * log5 is roughly 1.6, so absolute and relative error are the same. The result will be divided by 6, so a
			 * conservative error is the one already found in mc
			 */
			...
			/*
			 * summation of a series roughly proportional to (1/8)^k. Estimate count of terms to estimate the precision
			 * (drop the favorable additional 1/k here): 0.125^k <= 10^(-precision), so k*log10(0.125) <= -precision so
			 * k>= precision/0.903.
			 */
			...
			/*
			 * At this point one could either forward to the log(BigDecimal) signature (implemented) or decompose n into
			 * Ifactors and use an implemenation of all the prime bases. Estimate of the result; convert the mc argument
			 * to an absolute error eps log(n+errn) = log(n)+errn/n = log(n)+eps
			 */
			...
			/*
			 * Padd n with a number of zeros to trigger the required accuracy in the standard signature method
			 */
	...
	static public BigDecimal log(final Rational r, final MathContext mc) {
		...
		if (r.compareTo(Rational.ZERO) <= 0)
			...
			/*
			 * log(r+epsr) = log(r)+epsr/r. Convert the precision to an absolute error in the result. eps contains the
			 * required absolute error of the result, epsr/r.
			 */
			double eps = prec2err(Math.log(r.doubleValue()), mc.getPrecision());

			/*
			 * Convert this further into a requirement of the relative precision in r, given that epsr/r is also the
			 * relative precision of r. Add one safety digit.
			 */
	...
	static public BigDecimal pow(final BigDecimal x, final BigDecimal y) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			 ...
			 */
			double errR = Math.abs(logx.doubleValue() * y.ulp().doubleValue() / 2.) + Math.abs(y.doubleValue()
					* x.ulp().doubleValue() / 2. / x.doubleValue());
	...
	static public BigDecimal powRound(final BigDecimal x, final int n) {
		...
		if (n == 1)
			...
			/*
			 * The relative error in the result is n times the relative error in the input. The estimation is slightly
			 * optimistic due to the integer rounding of the logarithm. Since the standard BigDecimal.pow can only
			 * handle positive n, we split the algorithm.
			 */
	 ...
	 *            The base.
	 * @param n
	 *            The exponent. The current implementation allows n only in the interval of the standard int values.
	 * @return x^n.
	 * @since 2010-05-26
	 * @author Richard J. Mathar
	 */
	static public BigDecimal powRound(final BigDecimal x, final BigInteger n) {
		/**
		 * For now, the implementation forwards to the cases where n is in the range of the standard integers. This
		 * might, however, be implemented to decompose larger powers into cascaded calls to smaller ones.
		 */
	 ...
	 * 
	 * @param x
	 *            The base. Generally enforced to be positive, with the exception of integer exponents where the sign is
	 *            carried over according to the parity of the exponent.
	 * @param q
	 *            The exponent.
	 * @return x^q.
	 * @since 2010-05-26
	...
	static public BigDecimal powRound(final BigDecimal x, final Rational q) {
		...
		if (q.compareTo(BigInteger.ONE) == 0)
			...
			/*
			 * We are sure that the denominator is positive here, because normalize() has been called during constrution
			 * etc.
			 */
		 ...
		 */
			...
			if (q.isIntegerFrac()) {
				/*
				 * Newton method with first estimate in double precision. The disadvantage of this first line here is
				 * that the result must fit in the standard range of double precision numbers exponents.
				 */
				double estim = Math.pow(x.doubleValue(), q.doubleValue());
				BigDecimal res = new BigDecimal(estim);

				/*
				 * The error in x^q is q*x^(q-1)*Delta(x). The relative error is q*Delta(x)/x, q times the relative
				 * error of x.
				 */
				...
				/*
				 * The main point in branching the cases above is that this conversion will succeed for numerator and
				 * denominator of q.
				 */
				int qa = q.a.intValue();
				int qb = q.b.intValue();
				...
				for (;;) {
					/*
					 * numerator and denominator of the Newton term. The major disadvantage of this implementation is
					 * that the updates of the powers of the new estimate are done in full precision calling
					 * BigDecimal.pow(), which becomes slow if the denominator of q is large.
					 */
					...
					if (precDiv <= 0) {
						/*
						 * The case when the precision is already reached and any precision will do.
						 */
					...
					/*
					 * reached final precision if the relative error fell below reserr, |eps/res| < reserr
					 */
					if (eps.divide(res, MathContext.DECIMAL64).abs().compareTo(reserr) < 0) {
						/*
						 * delete the bits of extra precision kept in this working copy.
						 */
				...
				/*
				 * The error in x^q is q*x^(q-1)*Delta(x) + Delta(q)*x^q*log(x). The relative error is q/x*Delta(x) +
				 * Delta(q)*log(x). Convert q to a floating point number such that its relative error becomes
				 * negligible: Delta(q)/q << Delta(x)/x/log(x) .
				 */
	...
	static public BigDecimal sin(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (res.compareTo(p) > 0) {
				...
				/*
				 * for the range 0<=x<Pi/2 one could use sin(2x)=2sin(x)cos(x) to split this further. Here, use the sine
				 * up to pi/4 and the cosine higher up.
				 */
				if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
					...
					/*
					 * The error in the result is set by the error in x itself. We need at most k terms to squeeze
					 * x^(2k+1)/(2k+1)! below this value. x^(2k+1) < x.ulp; (2k+1)*log10(x) < -x.precision; 2k*log10(x)<
					 * -x.precision; 2k*(-log10(x)) > x.precision; 2k*log10(1/x) > x.precision
					 */
	...
	static public BigDecimal cos(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (res.compareTo(p) > 0) {
				...
				/*
				 * for the range 0<=x<Pi/2 one could use cos(2x)= 1-2*sin^2(x) to split this further, or use the cos up
				 * to pi/4 and the sine higher up. throw new ProviderException("Not implemented: cosine ") ;
				 */
				if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
					/*
					...
					/*
					 * The error in the result is set by the error in x^2/2 itself, xUlpDbl. We need at most k terms to
					 * push x^(2k+1)/(2k+1)! below this value. x^(2k) < xUlpDbl; (2k)*log(x) < log(xUlpDbl);
					 */
	...
	static public BigDecimal asin(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ONE) > 0 || x.compareTo(BigDecimal.ONE.negate()) < 0) {
			...
			for (int i = 1;; i++) {
				...
				BigDecimal c = divideRound(multiplyRound(xpowi, ifacN), ifacD.multiply(new BigInteger("" + (2 * i
						+ 1))));
				resul = resul.add(c);
				/*
	...
	static public BigDecimal atan(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			...
			/*
			 * Abramowitz-Stegun 4.4.34 convergence acceleration 2*arctan(x) = arctan(2x/(1-x^2)) = arctan(y).
			 * x=(sqrt(1+y^2)-1)/y This maps 0<=y<=3 to 0<=x<=0.73 roughly. Temporarily with 2 protectionist digits.
			 */
	...
	static public BigDecimal cosh(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (x.doubleValue() > 1.5) {
				...
				/*
				 * The error in the result is set by the error in x^2/2 itself, xUlpDbl. We need at most k terms to push
				 * x^(2k)/(2k)! below this value. x^(2k) < xUlpDbl; (2k)*log(x) < log(xUlpDbl);
				 */
				int k = (int) (Math.log(xUlpDbl) / Math.log(x.doubleValue())) / 2;

				/*
				 * The individual terms are all smaller than 1, so an estimate of 1.0 for the absolute value will give a
				 * safe relative error estimate for the indivdual terms
				 */
				MathContext mcTay = new MathContext(err2prec(1., xUlpDbl / k));
				for (int i = 1;; i++) {
					/*
	...
	static public BigDecimal sinh(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (x.doubleValue() > 2.4) {
				...
				/*
				 * The error in the result is set by the error in x itself. The first derivative of sinh(x) is cosh(x),
				 * so the absolute error in the result is cosh(x)*errx, and the relative error is coth(x)*errx =
				 * errx/tanh(x)
				 */
	...
	static public BigDecimal Gamma(final BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (x.compareTo(BigDecimal.ONE) != 0) {
				...
				for (int n = 2;; n++) {
					/*
					 * multiplying z^n/n by zeta(n-1) means that the two relative errors add. so the requirement in the
					 * relative error of zeta(n)-1 is that this is somewhat smaller than the relative error in z^n/n
					 * (the absolute error of thelatter is the absolute error in z)
					 */
					...
					/*
					 * At larger n, zeta(n)-1 is roughly 1/2^n. The product is c/2^n. The relative error in c is
					 * c.ulp/2/c . The error in the product should be small versus eps/10. Error from 1/2^n is
					 * c*err(sigma-1). We need a relative error of zeta-1 of the order of c.ulp/50/c. This is an
					 * absolute error in zeta-1 of c.ulp/50/c/2^n, and also the absolute error in zeta, because zeta is
					 * of the order of 1.
					 */
			...
			/*
			 * The relative error in the result is the absolute error in the input variable times the digamma (psi)
			 * value at that point.
			 */
	...
	static public BigDecimal Gamma(final Rational q, final MathContext mc) {
		if (q.isBigInteger()) {
			...
			/*
			 * The relative error of the result is psi(x)*Delta(x). Tune Delta(x) such that this is equivalent to mc:
			 * Delta(x) = precision/psi(x).
			 */
	...
	static public BigDecimal mod2pi(BigDecimal x) {
		/*
		 * write x= 2*pi*k+r with the precision in r defined by the precision of x and not compromised by the precision
		 * of 2*pi, so the ulp of 2*pi*k should match the ulp of x. First get a guess of k to figure out how many digits
		 * of 2*pi are needed.
		 */
		...
		/*
		 * Delegate the actual operation to the BigDecimal class, which may return a negative value of x was negative .
		 */
	...
	static public BigDecimal modpi(BigDecimal x) {
		/*
		 * write x= pi*k+r with the precision in r defined by the precision of x and not compromised by the precision of
		 * pi, so the ulp of pi*k should match the ulp of x. First get a guess of k to figure out how many digits of pi
		 * are needed.
		 */
	...
	static public BigDecimal zeta(final int n, final MathContext mc) {
		...
		if (n % 2 == 0) {
			...
			/*
			 * to be multiplied by pi^n. Absolute error in the result of pi^n is n times error in pi times pi^(n-1).
			 * Relative error is n*error(pi)/pi, requested by mc. Need one more digit in pi if n=10, two digits if n=100
			 * etc, and add one extra digit.
			 */
			...
			/*
			 * Broadhurst BBP <a href="http://arxiv.org/abs/math/9803067">arXiv:math/9803067</a> Error propagation: S31
			 * is roughly 0.087, S33 roughly 0.131
			 */
			...
			/*
			 * Broadhurst BBP <a href=http://arxiv.org/abs/math/9803067">arXiv:math/9803067</a> Error propagation: S51
			 * is roughly -11.15, S53 roughly 22.165, S55 is roughly 0.031 9*2048*S51/6265 = -3.28. 7*2038*S53/61651=
			 * 5.07. 738*2048*S55/61651= 0.747. The result is of the order 1.03, so we add 2 digits to S51 and S52 and
			 * one digit to S55.
			 */
			...
			/*
			 * The first term, including the facor (2pi)^n, is essentially most of the result, near one. The second term
			 * below is roughly in the range 0.003 to 0.009. So the precision here is matching the precisionn requested
			 * by mc, and the precision requested for 2*pi is in absolute terms adjusted.
			 */
			...
			if (n % 4 == 3) {
				/*
				 * since the argument n is at least 7 here, the drop of the terms is at rather constant pace at least
				 * 10^-3, for example 0.0018, 0.2e-7, 0.29e-11, 0.74e-15 etc for npr=1,2,3.... We want 2 times these
				 * terms fall below eps/10.
				 */
				int kmax = mc.getPrecision() / 3;
				eps /= kmax;
				/*
				 * need an error of eps for 2/(exp(2pi)-1) = 0.0037 The absolute error is
				...
				for (int npr = 2; npr <= kmax; npr++) {
					/*
					 * the error estimate above for npr=1 is the worst case of the absolute error created by an error in
					 * 2pi. So we can safely re-use the exp2p value computed above without reassessment of its error.
					 */
				...
				/*
				 * since the argument n is at least 9 here, the drop of the terms is at rather constant pace at least
				 * 10^-3, for example 0.0096, 0.5e-7, 0.3e-11, 0.6e-15 etc. We want these terms fall below eps/10.
				 */
				int kmax = (1 + mc.getPrecision()) / 3;
				eps /= kmax;
				/*
				 * need an error of eps for 2/(exp(2pi)-1)*(1+4*Pi/8/(1-exp(-2pi)) = 0.0096 at k=7 or = 0.00766 at k=13
				 * for example. The absolute error is 0.017*err(pi) at k=9, 0.013*err(pi) at k=13, 0.012 at k=17
				 */
	...
	static public double zeta1(final int n) {
		...
		final double[] zmin1 = { 0., 0., 6.449340668482264364724151666e-01, 2.020569031595942853997381615e-01,
				8.232323371113819151600369654e-02, 3.692775514336992633136548646e-02, 1.734306198444913971451792979e-02,
				8.349277381922826839797549850e-03, 4.077356197944339378685238509e-03, 2.008392826082214417852769232e-03,
				9.945751278180853371459589003e-04, 4.941886041194645587022825265e-04, 2.460865533080482986379980477e-04,
				1.227133475784891467518365264e-04, 6.124813505870482925854510514e-05, 3.058823630702049355172851064e-05,
				1.528225940865187173257148764e-05, 7.637197637899762273600293563e-06, 3.817293264999839856461644622e-06,
				1.908212716553938925656957795e-06, 9.539620338727961131520386834e-07, 4.769329867878064631167196044e-07,
				2.384505027277329900036481868e-07, 1.192199259653110730677887189e-07, 5.960818905125947961244020794e-08,
				2.980350351465228018606370507e-08, 1.490155482836504123465850663e-08, 7.450711789835429491981004171e-09,
				3.725334024788457054819204018e-09, 1.862659723513049006403909945e-09, 9.313274324196681828717647350e-10,
				4.656629065033784072989233251e-10, 2.328311833676505492001455976e-10, 1.164155017270051977592973835e-10,
				5.820772087902700889243685989e-11, 2.910385044497099686929425228e-11, 1.455192189104198423592963225e-11,
				7.275959835057481014520869012e-12, 3.637979547378651190237236356e-12, 1.818989650307065947584832101e-12,
				9.094947840263889282533118387e-13, 4.547473783042154026799112029e-13, 2.273736845824652515226821578e-13,
				1.136868407680227849349104838e-13, 5.684341987627585609277182968e-14, 2.842170976889301855455073705e-14,
				1.421085482803160676983430714e-14, 7.105427395210852712877354480e-15, 3.552713691337113673298469534e-15,
				1.776356843579120327473349014e-15, 8.881784210930815903096091386e-16, 4.440892103143813364197770940e-16,
				2.220446050798041983999320094e-16, 1.110223025141066133720544570e-16, 5.551115124845481243723736590e-17,
				2.775557562136124172581632454e-17, 1.387778780972523276283909491e-17, 6.938893904544153697446085326e-18,
				3.469446952165922624744271496e-18, 1.734723476047576572048972970e-18, 8.673617380119933728342055067e-19,
				4.336808690020650487497023566e-19, 2.168404344997219785013910168e-19, 1.084202172494241406301271117e-19,
				5.421010862456645410918700404e-20, 2.710505431223468831954621312e-20, 1.355252715610116458148523400e-20,
				6.776263578045189097995298742e-21, 3.388131789020796818085703100e-21, 1.694065894509799165406492747e-21,
				8.470329472546998348246992609e-22, 4.235164736272833347862270483e-22, 2.117582368136194731844209440e-22,
				1.058791184068023385226500154e-22, 5.293955920339870323813912303e-23, 2.646977960169852961134116684e-23,
				1.323488980084899080309451025e-23, 6.617444900424404067355245332e-24, 3.308722450212171588946956384e-24,
				1.654361225106075646229923677e-24, 8.271806125530344403671105617e-25, 4.135903062765160926009382456e-25,
				2.067951531382576704395967919e-25, 1.033975765691287099328409559e-25, 5.169878828456431320410133217e-26,
				2.584939414228214268127761771e-26, 1.292469707114106670038112612e-26, 6.462348535570531803438002161e-27,
				3.231174267785265386134814118e-27, 1.615587133892632521206011406e-27, 8.077935669463162033158738186e-28,
				4.038967834731580825622262813e-28, 2.019483917365790349158762647e-28, 1.009741958682895153361925070e-28,
				5.048709793414475696084771173e-29, 2.524354896707237824467434194e-29, 1.262177448353618904375399966e-29,
				6.310887241768094495682609390e-30, 3.155443620884047239109841220e-30, 1.577721810442023616644432780e-30,
		...
		if (n < zmin1.length)
			...
			/*
			 * Result is roughly 2^(-n), desired accuracy 18 digits. If zeta(n) is computed, the equivalent accuracy in
			 * relative units is higher, because zeta is around 1.
			 */
	 ...
	 * @param x
	 *            The main argument.
	 * @return psi(x). The error is sometimes up to 10 ulp, where AS 6.3.15 suffers from cancellation of digits and
	 *         psi=0
	 * @since 2009-08-26
	 * @author Richard J. Mathar
	 */
	static public double psi(final double x) {
		...
		if (x > 2.0) {
			/*
			 * Reduce to a value near x=1 with the standard recurrence formula. Abramowitz-Stegun 6.3.5
			 */
			int m = (int) (x - 0.5);
			double xmin1 = x - m;
			double resul = 0.;
			...
			/* 0.422... = 1 -gamma */
			return resul + 0.422784335098467139393487909917597568 + 0.5 / xmin1 - 1. / (1 - xmin1 * xmin1) - Math.PI
					/ (2. * Math.tan(Math.PI * xmin1));
		}
	} /* psi */
	 ...
	 *            Specification of the accuracy of the result
	 * @return S_(n,p)(a)
	 * @since 2009-08-09 <a href="http://arxiv.org/abs/math/9803067">arXiv:math/9803067</a>
	 * @author Richard J. Mathar
	 */
	static protected BigDecimal broadhurstBBP(final int n, final int p, final int a[], MathContext mc) {
		...
		/*
		 * Divide this through the number of terms in the sum to account for error accumulation The divisor 2^(p(k+1)/2)
		 * means that on the average each 8th term in k has shrunk by relative to the 8th predecessor by 1/2^(4p).
	...
	public static Rational toRational(BigDecimal x) {
		/*
		 * represent the floating point number by the exact rational variant of the current truncated representation
		 */
		int s = x.scale();
		if (s > 0)
	 ...
	 * 
	 * @param x
	 *            The number the absolute value of which will be decomposed.
	 * @return A list of the form [a0,a1,a2,a3,...] where The decomposition is |x| = a0+1/(a1+1/(a2+1/(a3+...))).
	 * @since 2012-03-09
	 * @author Richard J. Mathar
	 */
	...
	static public BigDecimal multiplyRound(final BigDecimal x, final BigDecimal y) {
		...
		/*
		 * The estimation of the relative error in the result is the sum of the relative errors |err(y)/y|+|err(x)/x|
		 */
	...
	static public BigDecimal divideRound(final BigDecimal x, final BigDecimal y) {
		...
		/*
		 * If x and y are precise integer values that may have common factors, the method above will truncate trailing
		 * zeros, which may result in a smaller apparent accuracy than starte... add missing trailing zeros now.
		 */
	 ...
	 *            The input value
	 * @param d
	 *            The (positive) value of zeros to be added as least significant digits.
	...
	/**
	 * Boost the precision by appending decimal zeros to the value. This returns a value which appears to have a higher
	 ...
	 * 
	 * @param x
	 *            The value of the variable The value returned depends only on the absolute value, not on the sign.
	 * @param xerr
	 *            The absolute error in the variable The value returned depends only on the absolute value, not on the
	 *            sign.
	 * @return The number of valid digits in x. Derived from the representation x+- xerr, as if the error was
	 *         represented in a "half width" (half of the error bar) form. The value is rounded down, and on the
	...
	static public int err2prec(double x, double xerr) {
		/*
		 * Example: an error of xerr=+-0.5 at x=100 represents 100+-0.5 with a precision = 3 (digits).
		 */
	 ...
	 * 
	 * @param xerr
	 *            The relative error in the variable. The value returned depends only on the absolute value, not on the
	 *            sign.
	 * @return The number of valid digits in x. The value is rounded down, and on the pessimistic side for that reason.
	 * @since 2009-08-05
	 * @author Richard J. Mathar
	...
	static public int err2prec(double xerr) {
		/*
		 * Example: an error of xerr=+-0.5 a precision of 1 (digit), an error of +-0.05 a precision of 2 (digits)
		 */
