...
/**
 * A BigSurdVec represents an algebraic sum or differences of values which each term an instance of BigSurd. This mainly
 * means that sums or differences of two BigSurd (or two BigSurdVec) can be represented (exactly) as a BigSurdVec.
 * 
 * @since 2012-02-15
...
public class BigSurdVec implements Comparable<BigSurdVec> {
	...
	/**
	 * Internal representation: Each term as a single BigSurd. The value zero is represented by an empty vector.
	 */
	...
	/**
	 * ctor given a periodic continued fraction. The value to be represented has a cont. fraction
	 * [a0,a1,a2,...,a0,a1,a2,...]
	 * 
	 * @param a
	 *            The coefficients a[0], a[1],.. of the period of the continued fraction.
	 * @since 2012-03-08
	 * @author Richard J. Mathar
	 */
	public BigSurdVec(Vector<BigInteger> a) throws NumberFormatException {
		/*
		 * this = a0+1/(a1+1/(a2+....) recursively unbundled by setting this=x in a rational polynomial
		 */
		if (a.size() == 0)
		...
		/*
		 * reached the bottom of the recursion. Solve numer(x)/denom(x) = x to obtain x as the solution of a quadratic
		 * equation. Need to solve numer.a0+number.a1*x = x*(denom.a0+denom.a1*x) which is numer.a0+number.a1*x =
		 * x*(denom.a0+denom.a1*x). This becomes defeq=0.
		 */
	...
	/**
	 * Combine terms that can be written as a single surd. This unites for example the terms sqrt(90) and sqrt(10) to
	 * 4*sqrt(10).
	 * 
	 * @since 2012-02-15
	 * @author Richard J. Mathar
	...
	protected void normalize() {
		...
		for (int j = 1; j < terms.size(); j++) {
			...
			for (int ex = 0; ex < newter.size(); ex++) {
				...
				/*
				 * try to merge terms[j] and newter[ex]. todo = r * v with r a rational number is needed. Replaces v
				 * with v+todo = v*(1+r) if this reduction works.
				 */
	...
	/**
	 * Compare algebraic value with oth. Returns -1, 0 or +1 depending on whether this is smaller, equal to or larger
	 * than oth.
	 * 
	 * @param oth
	...
	/**
	 * Sign function. Returns -1, 0 or +1 depending on whether this is smaller, equal to or larger than zero.
	 * 
	 * @return 0 or +-1.
	 * @since 2012-02-15
	 * @author Richard J. Mathar
	...
	public int signum() {
		/*
		 * the case of zero is unique, because no (reduced) vector of surds other than the one element 0 itself can
		 * add/subtract to zero.
		 */
		if (terms.size() == 0)
			return 0;
		...
		/*
		 * if all terms have a common sign: take that one offsig is the index of the first "offending" term in the sense
		 * that its sign doese not agree with the term[0].
		 */
		...
		/*
		 * if there are two terms (now known to have different sign): forward to the comparison of the two elements as
		 * BigSurds
		 */
		if (terms.size() == 2)
		...
		/*
		 * if there are three terms, move the one with the offending sign to the other side and square both sides (which
		 * looses the sign) to remove all but one surds. The difference of the squared sides contains at most two terms,
		 * which reduces to the case above. t(0)+t(offbar) <> -t(offs)
		 */
		if (terms.size() == 3) {
			BigSurdVec lhs;
			if (offsig == 2)
		...
		/*
		 * for a larger number of terms: take a floating point representation with a small but correct number of digits,
		 * and resume with the sign of that one.
		 */
	...
	public BigDecimal BigDecimalValue(MathContext mc) {
		...
		/*
		 * To reduce cancellation errors, loop over increasing local precision until we are stable to the required
		 * result. Keep the old (less precise) estimate in res[0], and the newer, more precise in res[1].
		 */
		...
		for (int addpr = 1;; addpr += 3) {
			...
			/*
			 * start with an explicitly set first value to avoid treating the initial zero as being a rounded zero
			 */
	...
	public BigSurdVec negate() {
		...
		/*
		 * no normalization step here, because the negation of all terms does not introduce new common factors
		 */
		return resul;
	} /* negate */
	...
	public BigSurdVec sqr() {
		/*
		 * Binomial expansion. First the sum of the terms squared, then 2 times the mixed products.
		 */
		BigSurdVec resul = new BigSurdVec();
	...
	/**
	 * Return a string in the format (number/denom)*()^(1/2). If the discriminant equals 1, print just the prefactor.
	 * 
