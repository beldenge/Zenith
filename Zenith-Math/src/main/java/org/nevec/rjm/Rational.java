...
/**
 * Fractions (rational numbers). They are ratios of two BigInteger numbers, reduced to coprime numerator and
 * denominator.
 * 
 * @since 2006-06-25
...
public class Rational implements Cloneable, Comparable<Rational> {
	 ...
	 * 
	 * @param str
	 *            the string. This either has a slash in it, separating two integers, or, if there is no slash, is
	 ...
	 * 
	 * @param str
	 *            the string. This either has a slash in it, separating two integers, or, if there is no slash, is just
	 *            representing the numerator.
	 * @param radix
	 *            the number base for numerator and denominator Warning: this does not yet test for a denominator equal
	 *            to zero
	 * @author Richard J. Mathar
	 */
	...
	/**
	 * ctor from a terminating continued fraction. Constructs the value of cfr[0]+1/(cfr[1]+1/(cfr[2]+...))).
	 * 
	 * @param cfr
	 *            The coefficients cfr[0], cfr[1],... of the continued fraction. An exception is thrown if any of these
	 *            is zero.
	 * @since 2012-03-08
	 * @author Richard J. Mathar
	 */
	public Rational(Vector<BigInteger> cfr) {
		if (cfr.size() == 0)
			...
			/*
			 * recursive this = cfr[0]+1/(cfr[1]+...) where cfr[1]+... = rec =rec.a/rec.b this = cfr[0]+rec.b/rec.a =
			 * (cfr[0]*rec.a+rec.b)/rec.a . Create a cloned version of references to cfr, without cfr[0]
			 */
	...
	public Rational clone() {
		/*
		 * protected access means this does not work return new Rational(a.clone(), b.clone()) ;
		 */
	...
	public Rational multiply(final Rational val) {
		...
		/*
		 * Normalization to an coprime format will be done inside the ctor() and is not duplicated here.
		 */
	 ...
	 * 
	 * @param r
	 *            the inverse of the exponent. 2 for the square root, 3 for the third root etc
	...
	public Rational pow(Rational exponent) throws NumberFormatException {
		...
		/*
		 * calculate (a/b)^(exponent.a/exponent.b) as ((a/b)^exponent.a)^(1/exponent.b) = tmp^(1/exponent.b)
		 */
	...
	public Rational divide(final Rational val) {
		...
		/*
		 * Reduction to a coprime format is done inside the ctor, and not repeated here.
		 */
		return (new Rational(num, deno));
	} /* Rational.divide */
	...
	public int compareTo(final Rational val) {
		/*
		 * Since we have always kept the denominators positive, simple cross-multiplying works without changing the
		 * sign.
		 */
	...
	public double doubleValue() {
		/*
		 * To meet the risk of individual overflows of the exponents of a separate invocation a.doubleValue() or
		 * b.doubleValue(), we divide first in a BigDecimal environment and convert the result.
		 */
	...
	public BigDecimal BigDecimalValue(MathContext mc) {
		...
		/*
		 * the problem with n.divide(d,mc) is that the apparent precision might be smaller than what is set by mc if the
		 * value has a precise truncated representation. 1/4 will appear as 0.25, independent of mc
		 */
	 ...
	 * 
	 * @return The list of a0, a1, a2,... in this =a0+1/(a1+1/(a2+1/(a3+...)))). If this here is zero, the list is
	 *         empty.
	 * @since 2012-03-09
	 * @author Richard J. Mathar
	 */
	 ...
	 * @return the sum of the inverses of the integers from 1 to n. H_1=1 for n=1, H_2=3/2 for n=2 etc. For values of n
	 *         less than 1, zero is returned.
	 * @since 2008-10-19
	 * @author Richard J. Mathar
	 * @author Richard J. Mathar
	 */
