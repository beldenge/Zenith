...
/**
 * Polynomial with integer coefficients. Alternatively to be interpreted as a sequence which has the polynomial as an
...
public class BigIntegerPoly implements Cloneable {
	/**
	 * The list of all coefficients, starting with a0, then a1, as in poly=a0+a1*x+a2*x^2+a3*x^3+...
	 */
	Vector<BigInteger>					a;
	 ...
	 *            degree to need this coefficient, the intermediate coefficients are set to zero.
	 * @param value
	 ...
	 * 
	 * @return the number of polynomial coefficients. Differs from the polynomial degree by one.
	 * @author Richard J. Mathar
	 */
	public int size() {
		return a.size();
	 ...
	 * 
	 * @return power of the smallest non-zero coefficient. If the polynomial is identical to 0, 0 is returned.
	 * @author Richard J. Mathar
	 */
	public int ldegree() {
	...
	/**
	 * Multiply by another polynomial. Equivalent to a convolution of the polynomial coefficients.
	 * 
	 * @param val
	 ...
	 * @param val
	 *            the other polynomial
	 * @return A vector with [0] containg the polynomial of degree which is the difference of the degree of this and the
	 *         degree of val. [1] the remainder polynomial. This = returnvalue[0] + returnvalue[1]/val .
	 * @since 2012-03-01
	 * @author Richard J. Mathar
	 */
	public BigIntegerPoly[] divideAndRemainder(final BigIntegerPoly val) {
		...
		if (thisSimpl.degree() < valSimpl.degree()) {
			...
			/*
			 * long division. Highest degree by dividing the highest degree of this thru val. At this point an exception
			 * is thrown if the polynomial division cannot be done with integer coefficients.
			 */
			ret[0] = new BigIntegerPoly();
	 ...
	 * 
	 * @return The representation a0+a1*x+a2*x^2+... The terms with zero coefficients are not mentioned.
	 * @since 2008-10-26
	 * @author Richard J. Mathar
	 */
	...
	/**
	 * Simplify the representation. Trailing values with zero coefficients (at high powers) are deleted.
	 * 
	 * @author Richard J. Mathar
	 */
	 ...
	 * 
	 * @return A (potentially empty) vector of factors, without multiplicity. Only factors with non-zero absolute
	 *         coefficient are generated. This means the factors are of the form x^2+a*x+b=0 with nonzero b.
	 * @since 2012-03-01
	 * @author Richard J. Mathar
	 */
	protected Vector<BigIntegerPoly> i2roots() {
		...
		/*
		 * Generate the floating point values of roots. To have some reasonable accuracy in the results, add zeros to
		 * the integer coefficients, scaled by the expected division with values of b (which are all <= a.firstele).
		 * Number of decimal digits in bsco by using a log2->log10 rough estimate and adding 6 safety digits
		 */
		...
		/*
		 * for each of the roots z try to see whether c*z^2+a*z+b=0 with integer a, b and c where b is restricted to a
		 * signed divisor of the constant coefficient. Solve z*(c*z+a)=-b or c*z+a = -b/z or -b/z-c*z = some integer a.
		 */
		for (BigComplex z : roo) {
			for (BigInteger bco : b)
				for (BigInteger cco : c) {
					/*
					 * the major reason to avoid the case b=0 is that this would require precaution of double counting
					 * below. Note that this case is already covered by using iroots().
					 */
					if (bco.signum() != 0) {
						for (int sig = -1; sig <= 1; sig += 2) {
							...
							/*
							 * test the polynomial remainder. if zero, add the term to the results.
							 */
	...
	/**
	 * Factorization into integer polynomials. The current factorization detects only factors which are polynomials of
	 * order up to 2.
	 * 
