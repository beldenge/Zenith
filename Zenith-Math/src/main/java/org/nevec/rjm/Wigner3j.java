...
/**
 * Exact representations of Wigner 3jm and 3nj values of half-integer arguments. R. J. Mathar,
 * <a href="http://arxiv.org/abs/1102.5125">Corrigendum to "Universal factorization of ..[J. Phys. A: Math. Gen.37
 * (2004) 3259]"</a> R. J. Mathar, <a href="http://vixra.org/abs/1202.0093">Symmetries in Wigner 18-j and 21-j
 * Symbols</a>
 * 
 * @since 2011-02-15
...
public class Wigner3j {
	 ...
	 * The first command line argument is one of the three tags which determine whether a 3jm, a 6j or a 9j symbol will
	 * be computed. The other arguments are 6 or 9 integer values, which are the physical (half-integer) values
	 * multplied by 2 and augmented by 1. The order of the 6 or 9 values is as reading the corresponding standard symbol
	 * as first row, then second row (and for the 9j symbol) third row.
	 * 
	 * @param args
	 *            The tag for the type of computation followed by the integer parameters.
	 * @since 2011-02-15
	 * @author Richard J. Mathar
	 */
	...
	/**
	 * The Wigner 3jm symbol (j1,j2,j3,m1,m2,m3). All arguments of the function are the actual parameters multiplied by
	 * 2, so they all allow an integer representation.
	 * 
	 * @param j1
	 ...
	 * @return The value of the symbol. Zero if any of the triangular inequalities is violated or some parameters are
	 *         out of range.
	 * @since 2011-02-13
	 * @author Richard J. Mathar
	 */
	...
	/**
	 * Wigner 3jn symbol. For the 6j symbol, the input of the 3 lines is "1 2 3 1 5 6", "4 5 3 4 2 6" "2j1+1 2j2+1 2j3+1
	 * 2l1+1 2l2+1 2l3+1"
	 * 
	 * @param m1
	 *            The information on the number of angular momenta.
	 * @param t1
	 *            The list of one half of the triads, indexing j, whitespace separated
	 * @param t2
	 *            The list of the second half of the triads, indexing j, whitespace separated
	 * @param j
	 *            The list of the integer values of the angular momenta. They are actually the doubled j-values plus 1,
	 *            whitespace separated. Only as many as announced by the m1 parameter are used; trailing numbers are
	 *            ignored. A. Bar-Shalom and M. Klapisch,
	 *            <a href="http://dx.doi.org/10.1016/0010-4655(88)90192-0">NJGRAF...</a>, Comp. Phys Comm. 50 (3) (1988)
	 *            375
	...
	static public BigSurdVec wigner3j(String m1, String t1, String t2, String j) {
		/*
		 * The first number in the line "m" is the number of angular momenta. The rest of the line is ignored.
		 */
		...
		/*
		 * Scan the numbers in the line "j". Excess numbers beyond what has been announced in the "m" line are ignored.
		 */
		int[] jvec = new int[m];
		...
		/*
		 * Basic sanity checks. All indices in the first two lines address a number in the third line, and each index
		...
		/*
		 * maintain a 0-based index which shows where the j-value has its first and second occurrence in the flattened
		 * list of triads.
		 */
		...
		/*
		 * Move on from the 2j+1 values of the input to the j-values. Subtract one and divide through 2.
		 */
	...
	/**
	 * Wigner 3jn symbol. Computes sum_{mi} (-1)^(j1-m1+j2-m2+...) triad(triadidx[0..2])*triad(triadidx[3..5])*... where
	 * each factor is a Wigner-3jm symbol with each sign of m_i occurring once at the corresponding l-value.
	 * 
	 * @param tvec
	 * @param J
	 *            The list of J-values
	 * @param M
	 *            The list of M-values associated with the J. This contains null where the parameter has not yet been
	 *            set by an outer loop.
	 * @param triadidx
	...
	static private BigSurdVec wigner3j(final int[] tvec, final Rational[] J, final Rational[] M, final int[] triadidx) {
		/*
		 * The result of the computation. The sum over all m-combinations of the triads.
		 */
		BigSurdVec res = new BigSurdVec();

		/*
		 * First step is to monitor the triangular conditions on the J. If at least one is violated, the result is zero.
		 * Loop over the triads.
		 */
		...
		/*
		 * the index of the preferred member of the triad list. Preference given to those dangling in triads where
		 * alreaday two others are fixed, then to members where at least one is fixed, then to smallest associated
		 * J-values.
		 */
		int freeM = -1;
		int freeMrank = -1;
		for (int i = 0; i < triadidx.length; i++) {
			...
			if (M[triadidx[i]] == null) {
				/*
				 * two cases: value is fixed implicitly because already two others values are set in the triad. or it is
				 * still to maintain its own explicit loop.
				 */
				int triadn = i / 3;
				int triadr = i % 3;
				/*
		...
		if (freeM >= 0) {
			...
			if (M[triadidx[freeM]] == null) {
				...
				if (M[triadidx[nei1]] == null || M[triadidx[nei2]] == null) {
					...
					/*
					 * Set its value and the value at its companion j-value. Sum of the three m-values in the triad is
					 * to be zero for a non-zero contribution.
					 */
					...
					/*
					 * No contribution if the m-value enforced by the other two entries is outside the range -|J|..|J|
					 * enforced by its associated J-value. One could essentially remove this branching and let
					 * wigner3j() decide on this, but this is inefficient.
					 */
					...
					/*
					 * zero contribution if this m-value cannot be set to any value compatible with the triangular
					 * conditions.
					 */
				}
				return res;
		...
		/*
		 * reached the bottom of the loop where all M-values are assigned. Build the product over all Wigner-3jm values
		...
		for (int ji = 0; ji < triadidx.length; ji += 3) {
			...
			/*
			 * if a partial product yields zero, the total product is zero, too, and offers an early exit.
			 */
			if (res.signum() == 0)
		...
		/*
		 * sign depends on the sum being even or odd. We assume that "sig" is integer and look only at the numerator
		 */
	...
	/**
	 * The Wigner 3jm symbol (j1,j2,j3,m1,m2,m3). Warning: there is no check that each argument is indeed half-integer.
	 * 
	 * @param j1
	...
	static protected BigSurd wigner3jm(Rational j1, Rational j2, Rational j3, Rational m1, Rational m2, Rational m3) {
		...
		/*
		 * It is tacitly assumed that all the major j_i, m_i values are in the integer range. This is implicitly
		 * plausible since otherwise the execution times of the following loop over the k-values would be immense.
		 */
