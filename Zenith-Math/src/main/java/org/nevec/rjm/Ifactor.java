...
public class Ifactor implements Cloneable, Comparable<Ifactor> {
	 ...
	 * primeexp[0]^primeexp[1]*primeexp[2]^primeexp[3]*... The value 0 is represented by an empty vector, the value 1 by
	 * a vector of length 1 with a single power of 0.
	 */
	 ...
	 * 
	 * @param pows
	 *            the vector with the sorted list of exponents. pows[0] is the exponent of 2, pows[1] the exponent of 3,
	 *            pows[2] the exponent of 5 etc. Note that this list does not include the primes, but assumes a
	...
	public Ifactor multiply(final Ifactor oth) {
		/*
		 * This might be done similar to the lcm() implementation by adding the powers of the components and calling the
		 * constructor with the list of exponents. This here is the simplest implementation, but slow because it calls
		 * another prime factorization of the product: return( new Ifactor(n.multiply(oth.n))) ;
		 */
	...
	protected Ifactor multGcdLcm(final Ifactor oth, int type) {
		...
		if (primeexp.size() != 0 && oth.primeexp.size() != 0) {
			/*
			 * Cases of 1 times something return something. Cases of lcm(1, something) return something. Cases of gcd(1,
			 * something) return 1.
			 */
			if (primeexp.firstElement().intValue() == 1 && type == 0)
				...
				while (idxOth < oth.primeexp.size() || idxThis < primeexp.size()) {
					if (idxOth >= oth.primeexp.size()) {
						/*
						 * exhausted the list in oth.primeexp; copy over the remaining 'this' if multiplying or lcm,
						 * discard if gcd.
						 */
						if (type == 0 || type == 2) {
						...
						case 0:
							...
							case 0:
								...
								ex = primeexp.elementAt(idxThis + 1).intValue() + oth.primeexp.elementAt(idxOth
										+ 1).intValue();
								break;
							case 1:
								/* gcd means minimum of exponents */
								ex = Math.min(primeexp.elementAt(idxThis + 1).intValue(), oth.primeexp.elementAt(idxOth
										+ 1).intValue());
								break;
							default:
								/* lcm means maximum of exponents */
								ex = Math.max(primeexp.elementAt(idxThis + 1).intValue(), oth.primeexp.elementAt(idxOth
										+ 1).intValue());
								break;
							}
							prod.primeexp.add(p);
	...
	public Ifactor divide(final Ifactor oth) {
		/*
		 * todo: it'd probably be faster to cancel the gcd(this,oth) first in the prime power representation, which
		 * would avoid a more strenous factorization of the integer ratio
		 */
	...
	public Ifactor pow(final int exponent) throws ArithmeticException {
		...
		/*
		 * general case, the vector with the prime factor powers, which are component-wise exponentiation of the
		 * individual prime factor powers.
		 */
	 ...
	 * @return n^(1/r). The return value falls into the Ifactor class if r is positive, but if r is negative a Rational
	 *         type is needed.
	 * @since 2009-05-18
	 * @author Richard J. Mathar
	 */
	public Rational root(final int r) throws ArithmeticException {
		if (r == 0)
			...
			for (int i = 0; i < primeexp.size(); i += 2) {
				/*
				 * all exponents must be multiples of r to succeed (that is, to stay in the range of rational results).
				 */
				int ex = primeexp.elementAt(i + 1).intValue();
				if (ex % r != 0)
	...
	public Vector<BigInteger> divisors() {
		/*
		 * Recursive approach: the divisors of p1^e1*p2^e2*..*py^ey*pz^ez are the divisors that don't contain the factor
		 * pz, and the divisors that contain any power of pz between 1 and up to ez multiplied by 1 or by a product that
		 * contains the factors p1..py.
		 */
		...
		if (n.compareTo(BigInteger.ONE) > 0) {
			...
			/*
			 * the output contains all products of the form partd[]*pz^ez, ez>0, and with the exception of the 1, all
			 * these are appended.
			 */
	 ...
	 * 
	 * @param k
	 *            The exponent of the powers.
	 * @return the sum of all divisors of the number, 1^k+....+n^k.
	 * @author Richard J. Mathar
	 */
	public Ifactor sigma(int k) {
		/*
		 * the question is whether keeping a factorization is worth the effort or whether one should simply multiply
		 * these to return a BigInteger...
		 */
		if (n.compareTo(BigInteger.ONE) == 0)
			return ONE;
			...
			/*
			 * multiplicative: sigma_k(p^e) = [p^(k*(e+1))-1]/[p^k-1] sigma_0(p^e) = e+1.
			 */
			...
			for (int i = 0; i < primeexp.size(); i += 2) {
				...
				if (k == 0)
					...
					/*
					 * This division is of course exact, no remainder The costly prime factorization is hidden here.
					 */
	...
	public Ifactor dropPrime() {
		...
		/*
		 * The cases n>1 Start empty. Copy all but the last factor over to the result the vector with the new prime
		 * factor powers, which contain the old prime factor powers up to but not including the last one.
		 */
		Ifactor pows = new Ifactor(0);
	...
	/**
	 * The Moebius function. 1 if n=1, else, if k is the number of distinct prime factors, return (-1)^k, else, if k has
	 * repeated prime factors, return 0.
	 * 
