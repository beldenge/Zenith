...
public class BigIntegerMath {
	...
	static public BigInteger binomial(final int n, final int k) {
		...
		BigInteger n2 = bin;
		for (BigInteger i = BigInteger.valueOf((long) (k
				- 1)); i.compareTo(BigInteger.ONE) >= 0; i = i.subtract(BigInteger.ONE))
	...
	static public BigInteger binomial(final BigInteger n, final BigInteger k) {
		...
		/*
		 * the following version first calculates n(n-1)(n-2)..(n-k+1) in the first loop, and divides this product
		 * through k(k-1)(k-2)....2 in the second loop. This is rather slow and replaced by a faster version below
		 * BigInteger n2 = bin ; BigInteger i= k.subtract(BigInteger.ONE) ; for( ; i.compareTo(BigInteger.ONE) >= 0 ; i
		 * = i.subtract(BigInteger.ONE) ) bin = bin.multiply(n2.subtract(i)) ; i= BigInteger.valueOf(k) ; for( ;
		 * i.compareTo(BigInteger.ONE) == 1 ; i = i.subtract(BigInteger.ONE) ) bin = bin.divide(i) ;
		 */

		/*
		 * calculate n then n(n-1)/2 then n(n-1)(n-2)(2*3) etc up to n(n-1)..(n-k+1)/(2*3*..k) This is roughly the best
		 * way to keep the individual intermediate products small and in the integer domain. First replace C(n,k) by
		 * C(n,n-k) if n-k<k.
		 */
		BigInteger truek = k;
		...
		/*
		 * Calculate C(num,truek) where num=n and truek is the smaller of n-k and k. Have already initialized
		 * bin=n=C(n,1) above. Start definining the factorial in the denominator, named fden
		 */
	 ...
	 * @return phi(n) <a href="http://oeis.org/A000010">A000010</a> in the OEIS.
	 * @since 2008-10-14
	 ...
	 * 
	 * @param n
	 *            The non-negative argument. Arguments less than zero throw an ArithmeticException.
	 * @return The integer square root, the square root rounded down.
	 * @since 2010-08-27
	 * @author Richard J. Mathar
	 */
	...
	static public BigInteger isqrt(final BigInteger n) {
		...
		while (true) {
			...
			/*
			 * Newton algorithm. This correction is on the low side caused by the integer divisions. So the value
			 * required may end up by one unit too large by the bare algorithm, and this is caught above by comparing
			 * x^2, (x+-1)^2 with n.
			 */
	 ...
	 *            The matrix.
	 * @param r
	 *            The row index of the row to be removed (0-based). An exception is thrown if this is outside the range
	 *            0 to the upper row index of A.
	 * @param c
	 *            The column index of the column to be removed (0-based). An exception is thrown if this is outside the
	 ...
	 * 
	 * @param A
	 *            The square matrix. If column and row dimensions are unequal, an ArithmeticException is thrown.
	 ...
	 * 
	 * @param A
	 *            The square matrix. If it is not of full rank, an ArithmeticException is thrown.
	 * @param rhs
	 *            The right hand side. The length of this vector must match the matrix size; else an ArithmeticException
	 *            is thrown.
	 ...
	 * 
	 * @param c
	 *            Represents the coefficients c[0]+c[1]*x+c[2]*x^2+.. of the polynomial
	 * @param x
	 ...
	 * @return t(n,k)
	 * @since 2009-08-06
	 * @author Richard J. Mathar <a href="http://dx.doi.org/10.1080/01630568908816313">P. L. Butzer et al, Num. Funct.
	 *         Anal. Opt. 10 (5)( 1989) 419-488</a>
	 */
